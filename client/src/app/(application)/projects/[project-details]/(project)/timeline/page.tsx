...
function splitTasks(tasks:Task[]) {
  const sortedTasks = tasks.sort((taskA, taskB) => time(taskA.createdOn) - time(taskB.createdOn))
  const result: Task[][] = [];
  ...
  for (const task of sortedTasks) {
    ...
    for (const row of result) {
      ...
      if (lastTask && time(task.createdOn) >= time(lastTask.dueDate) + 355_000_000) {
        row.push(task);
        added = true;
        break;
      }
...
export default function TimelinePage({
  ...
  return (
    <div className=" overflow-y-hidden h-screen" style={{scrollbarWidth: "none"}}>
      <ProjectHeader project={project} creator={creator}/>
      <ProjectNav projectId={projectId} remove="timeline"/>
      
      <div className="gap-1 w-full h-full relative rounded-lg mt-3 bg-slate-200 p-1" id="AAA">
        ...
        <br/>
        <div
          style={{left: (((new Date()).getTime() - 1704067200000) / 316224000) + "%"}}
        ...
        taskes.map( task => (
          <div
            ...
            style = {{
              ...
              top: index * 38 + "px",
              minWidth: ((time(task.dueDate) - time(task.createdOn)) / 316224000) + "%"
            }}
            className={`
              ...
              hover:bg-gray-700 hover:text-${getColour(task.priority)}-500 hover:rounded-md hover:w-fit hover:z-50
